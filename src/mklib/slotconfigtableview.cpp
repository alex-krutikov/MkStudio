...
void SlotConfigTableView::handleClick(QModelIndex ind)
{
    if (ind.column() == 3)
    {
        if (ind.data(Qt::DisplayRole).toString() == "")
            return; // no need to show editor in such case
        emit edit(ind);
    }
}
...
//==============================================================================
QWidget *
MBConfigWidgetItemDelegate::createEditor(QWidget *parent,
                                         ...
                                         const QModelIndex &index) const
{
    ...
    if (index.data(Qt::DisplayRole).toString().isEmpty())
        return 0; // no need to show editor in such case
    ...
    for (int i = 1; i < mb.length(); i++)
    {
        if (!arduinoOnly || mb.isArduino())
        {
            ...
            if (mb.isRegister())
                w->setItemData(i - 1, QColor("bisque"), Qt::BackgroundRole);
            if (mb.isExtendedRegister())
                w->setItemData(i - 1, QColor("lavender"), Qt::BackgroundRole);
        }
        mb.step();
    }
    w->setCurrentIndex(w->findText(current_text));
    connect(w, SIGNAL(currentIndexChanged(int)), this,
            SLOT(commitAndCloseEditor())); // otherwise user needs to click
...
//
//==============================================================================
void MBConfigWidgetItemDelegate::updateEditorGeometry(
...
//
//==============================================================================
void MBConfigWidgetItemDelegate::setModelData(QWidget *editor,
