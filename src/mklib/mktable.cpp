...
struct MKTable::MKTableAssignData
{
    ...
    static bool lessThan(const MKTableAssignData &s1,
                         const MKTableAssignData &s2);
};
...
{
    return QSize(horizontalHeader()->length() + 70 + verticalHeader()->width(),

                 verticalHeader()->length() + 70
...
bool MKTable::loadConfiguration(const QDomDocument &doc)
{
    ...
    for (i = 0; i < items_list.count(); i++)
    {
        ...
        if (str.contains("align_left"))
            titem->setTextAlignment(Qt::AlignVCenter | Qt::AlignLeft);
        if (str.contains("align_center"))
            titem->setTextAlignment(Qt::AlignVCenter | Qt::AlignHCenter);
        if (str.contains("align_right"))
        ...
        if (!str.isEmpty())
        {
            ...
            if (color.isValid()) { titem->setData(Qt::BackgroundRole, color); }
        }
        str = element.attribute("ForegroundColor");
        if (!str.isEmpty())
        {
            color.setNamedColor(str);
            if (color.isValid()) { titem->setData(Qt::ForegroundRole, color); }
        }
        str = element.attribute("SSSelector");
        if (!str.isEmpty()) { titem->setData(SSSelectorRole, str); }
...
void MKTable::saveConfiguration(QDomDocument &doc)
{
    ...
    for (i = 0; i < rowCount(); i++)
    {
        for (j = 0; j < columnCount(); j++)
        {
            ...
            if (titem->textAlignment() & Qt::AlignHCenter)
                str += "align_center;";
...
//
//==============================================================================
bool MKTable::MKTableAssignData::lessThan(const MKTableAssignData &s1,
                                          const MKTableAssignData &s2)
{
    bool b = false;
    if (s1.m_index < s2.m_index)
        b = true;
    ...
    if (m == MKTable::Polling)
    {
            ...
            for (j = 0; j < columnCount(); j++)
            {
                ...
                if (!str.isEmpty())
                {
                    ...
                    } else
                    {
                        unknown_selectors
                            << "\"" + str + "\" <"
                                   + titem->data(AssignRole).toString() + ">";
                    }
                }
        ...
        if (unknown_selectors.size())
        {
            QMessageBox::warning(this, "Предупреждение",
                                 "Некоторые используемые в таблице селекторы "
                                 "не найдены в листе настроек!\n\n"
...
void MKTable::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    if (si.count() > 4)
    {
        QMessageBox::information(this, "Ошибка",
                                 "Должно быть выделено не более 4 ячеек.");
        return;
    }
    ...
    { //---------- вывод графика
      //------------------------------------------------------------------------
        ...
        foreach (QTableWidgetItem *t, si)
        {
            ...
            {
                str_column = model()->headerData(j, Qt::Horizontal).toString();
            }
            //-------------------- название строки --------
            i = t->row();
            ...
            case 0:
                ...
                break;
            case 1:
                str += "зеленый самописец:";
                break;
            case 2:
                str += "красный самописец:";
                break;
            case 3:
                str += "черный самописец:  ";
                break;
            }
            si_count++;
            //-------------------- вывод графика ----------
            ...
            if (!str_column.isEmpty())
                str += " ( " + str_column.simplified() + " )\n";
            //-------------------- определение заголовка ----------
            if (si.count() == 1)
            {
                if (!str_row.isEmpty())
                    str_title += " ( " + str_row.simplified() + " )";
                if (!str_column.isEmpty())
                    str_title += " ( " + str_column.simplified() + " )";
            }
        }
        str.remove(str.length() - 1, 1);
        bool is_minimize = (size() == QSize(0, 0)) ? true : false;
        Plot *plot
            = new Plot(str_title, si, is_minimize, this,
                       mbmaster); // si.value(0)->data(AssignRole).toString() );
        ...
        connect(plot, SIGNAL(signalMinimize(bool)), this,
                SLOT(slotMinimize(bool)));
        connect(this, SIGNAL(signalCloseAllRecorders()), plot, SLOT(close()));
        connect(this, SIGNAL(signalMinimizeStateChange(bool)), plot,
                SLOT(updateMinimizeButtonState(bool)));
...
void MKTable::mouseReleaseEvent(QMouseEvent *event)
{
    if (mode == MKTable::Polling)
    {
        if (event->button() == Qt::LeftButton)
        {
            ...
            if (titem)
            {
                currentItemConfirmEdit
                    = titem->data(MKTable::SSConfirmEditRole).toBool();
                editItem(titem);
            }
        }
    }
...
void MKTable::keyPressEvent(QKeyEvent *event)
{
    if (hotkey.contains(event->key()))
    {
        ...
        case ShortCut::WriteValue: {
            mbmaster->setSlotValue(sc->assign().at(0), sc->assign().at(1),
                                   sc->assign().at(2),
                                   MMValue::fromInt(sc->variant()));
            refresh();
        }
        break;
        default:
...
void MKTable::refresh()
{
    ...
    cycle_begin:
        ...
        {
            vflag = false;
            for (j = i; (j < assign_data_size) && (assign_data[j].m_index == mm)
                        && (assign_data[j].s_index == ss);
                 j++)
            {
        ...
        for (; i < assign_data_size; i++)
        {
            ...
            if (assign_data[i].first_pass_flag
                || (value != assign_data[i].current_value))
            { // обновление
                ...
                if (value.status() == MMValue::Ok)
                {
                    ...
                    {
                        str = format_output(value, slot.datatype,
                                            assign_data[i].format);
                    } else
                    {
                        QVector<MapEmul> &m
                            = ss_enum[assign_data[i].ss_enum_index].map_display;
                        MapEmul me;
                        QString strValue;
                        foreach (me, m)
                        {
...
//
//==============================================================================
QString MKTable::format_output(const MMValue &value, const MBDataType &datatype,
                               int type)
{
    QString str;
    char *cf;
    ...
    case (MBDataType::BitsArduino):
        datatype_id = MBDataType::Bits;
        break;
    case (MBDataType::BytesArduino):
        datatype_id = MBDataType::Bytes;
        break;
    case (MBDataType::WordsArduino):
        datatype_id = MBDataType::Words;
        break;
...
//
//==============================================================================
void MKTableItemDelegate::paint(QPainter *painter,
...
//
//==============================================================================
QWidget *MKTableItemDelegate::createEditor(QWidget *parent,
                                           const QStyleOptionViewItem &option,
                                           const QModelIndex &index) const
{
    Q_UNUSED(option);
    ...
    if (table->mode == MKTable::Polling)
    {
        ...
        if (i >= 0)
        {
            ...
            connect(w, SIGNAL(editingFinished()), this,
...
//
//==============================================================================
void MKTableItemDelegate::updateEditorGeometry(
    ...
    const QModelIndex &index) const
{
    ...
    if (i >= 0)
    {
        ...
        QSize sz = w->size();
        int hh
            = (w->sizeHintForRow(0) + 2 * w->spacing()) * qMin(w->count(), 15);
        hh += w->spacing()
            + w->style()->pixelMetric(QStyle::PM_DefaultFrameWidth, &opt, w)
                  * 2;
        hh += w->style()->pixelMetric(QStyle::PM_MenuVMargin, &opt, w) * 2;
        int ww = w->sizeHintForColumn(0);
        ww += w->style()->pixelMetric(QStyle::PM_DefaultFrameWidth, &opt, w)
            * 2;
...
//
//==============================================================================
void MKTableItemDelegate::setModelData(QWidget *editor,
