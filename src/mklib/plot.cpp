...
Plot::Plot(QString title, QList<QTableWidgetItem *> mkItemList, bool min_flag,
           QWidget *parent, MBMasterXMLPtr mbmaster)
    : QWidget(parent)
    , ui(new Ui::Plot)
{
    QT_TRY
    {
        ...
        picker->setWindow(
            ui->cb_speed->itemData(ui->cb_speed->currentIndex(), Qt::UserRole)
                .toString());
        picker->setPlot(this);
        ...
        pb_export->setSizePolicy(QSizePolicy::Preferred,
        ...
        pb_minimize->setSizePolicy(QSizePolicy::Preferred,
                                   QSizePolicy::Preferred);
        pb_minimize->setStyleSheet(tb_qss);
        connect(pb_minimize, SIGNAL(clicked()), this,
                SLOT(pb_minimize_clicked()));
        ...
        connect(pb_grid, SIGNAL(toggled(bool)), this,
                SLOT(pb_grid_toggled(bool)));
        ...
        pb_crosshair->setSizePolicy(QSizePolicy::Preferred,
                                    QSizePolicy::Preferred);
        pb_crosshair->setStyleSheet(tb_qss);
        connect(pb_crosshair, SIGNAL(toggled(bool)), this,
                SLOT(pb_crosshair_toggled(bool)));
        ...
        pb_picker_data->setSizePolicy(QSizePolicy::Preferred,
                                      QSizePolicy::Preferred);
        pb_picker_data->setStyleSheet(tb_qss);
        connect(pb_picker_data, SIGNAL(toggled(bool)), this,
                SLOT(pb_picker_data_toggled(bool)));

        connect(ui->cb_plot_stat, SIGNAL(currentIndexChanged(int)), this,
                SLOT(change_current_plot(int)));

        connect(ui->pb_set_file, SIGNAL(clicked()), this,
                SLOT(set_file_for_write()));
        connect(ui->pb_file_write, SIGNAL(clicked()), this,
                SLOT(file_write_start_stop()));
        connect(ui->tb_isr_set, SIGNAL(clicked()), this,
                SLOT(tb_isr_set_clicked()));
        ...
        if (!timerId)
            QMessageBox::critical(this, "самописец",
                                  "Не удалось запустить самописец.");
    }
    QT_CATCH(...) {}
}
...
void Plot::closeEvent(QCloseEvent *event)
{
    QT_TRY
    {
        ...
        if (!file.fileName().isEmpty())
            openFilesList.removeOne(file.fileName());
...
void Plot::pb_export_clicked()
{
    QT_TRY
    {
        ...
        while (i >= (y_data_len - points_counter))
        {
            ...
            if (plots_count > 1)
                str += "\t" + QString::number(y_data2[j], 'g', 8);
            if (plots_count > 2)
                str += "\t" + QString::number(y_data3[j], 'g', 8);
            if (plots_count > 3)
                str += "\t" + QString::number(y_data4[j], 'g', 8);
            str += "\n";
            i--;
            j++;
            k--;
        ...
        QMessageBox::information(this, "Самописец",
...
void Plot::on_sb_plot_pen_w_valueChanged(int width)
{
    ...
    if (plots_count > 1) { plot_data2->setPen(QPen(Qt::green, width)); }
    if (plots_count > 2) { plot_data3->setPen(QPen(Qt::red, width)); }
    if (plots_count > 3) { plot_data4->setPen(QPen(Qt::black, width)); }
    ui->plot->replot();
}
//==============================================================================
//
...
void Plot::change_current_plot(int index)
{
    QT_TRY
    {
        ...
        case 0:
            ...
            break;
        case 1:
            y_data_stat = y_data2.get();
            break;
        case 2:
            y_data_stat = y_data3.get();
            break;
        case 3:
            y_data_stat = y_data4.get();
            break;
        default:
            y_data_stat = y_data1.get();
            break;
        ...
        if ((plots_count != 0) && (pause_flag == true)) { calc_statistic(); }
    }
    QT_CATCH(...) {}
}
//==============================================================================
    ...
    QT_TRY
    {
        ...
        if ((y_mean != 0) && (y_max != y_min) && isr_params.count())
        {
            if (isr_use_base_flag)
            {
                ...
                {
                    MMValue eb_v = mbmaster->getSlotValue(
                        isr_base.m, isr_base.s, isr_base.n - 1);
                    if (eb_v.isValid())
                    {
                ...
                ui->le_input_signal_range->setText(
                    QString::number(eb_ref, 'g', 8));
            }
...
void Plot::timerEvent(QTimerEvent *event)
{
    QT_TRY
    {
        ...
        for (i = 0; i < plots_count; i++)
        {
            value = mbmaster->getSlotValue(module_index[i], slot_index[i],
                                           value_index[i]);
        ...
        if (ui->cb_use_match->isChecked() && (koeff != 0))
        {
            if (plot_index != 0)
            {
                if (y_min1 != y_max1)
                {
                    for (i = 0; i < y_data_len; i++)
                    {
                        y_data1[i] = (koeff * (y_data1[i] - y_min1)
                                      / (y_max1 - y_min1))
                                   + y_mn;
                    }
                } else
            ...
            if (plot_index != 1)
            {
                if (y_min2 != y_max2)
                {
                    ...
                    {
                        y_data2[i]
                            = koeff * (y_data2[i] - y_min2) / (y_max2 - y_min2)
                            + y_mn;
                    }
                } else
                {
            ...
            if ((plots_count > 2) && (plot_index != 2))
            {
                if (y_min3 != y_max3)
                {
                    for (i = 0; i < y_data_len; i++)
                    {
                        y_data3[i] = (koeff * (y_data3[i] - y_min3)
                                      / (y_max3 - y_min3))
                                   + y_mn;
                    }
                } else
            ...
            if ((plots_count > 3) && (plot_index != 3))
            {
                if (y_min4 != y_max4)
                {
                    for (i = 0; i < y_data_len; i++)
                    {
                        y_data4[i] = (koeff * (y_data4[i] - y_min4)
                                      / (y_max4 - y_min4))
                                   + y_mn;
                    }
                } else
        ...
        if ((plots_count > 1) && (koeff != 0) && ui->cb_use_match->isChecked())
        {
            if (plot_index != 0)
            {
                if (y_min1 != y_max1)
                {
                    ...
                    {
                        y_data1[i]
                            = ((y_data1[i] - y_mn) * (y_max1 - y_min1) / koeff)
                            + y_min1;
                    }
                } else
                {
            ...
            if (plot_index != 1)
            {
                if (y_min2 != y_max2)
                {
                    ...
                    {
                        y_data2[i]
                            = ((y_data2[i] - y_mn) * (y_max2 - y_min2) / koeff)
                            + y_min2;
                    }
                } else
                {
            ...
            if ((plots_count > 2) && (plot_index != 2))
            {
                if (y_min3 != y_max3)
                {
                    ...
                    {
                        y_data3[i]
                            = ((y_data3[i] - y_mn) * (y_max3 - y_min3) / koeff)
                            + y_min3;
                    }
                } else
                {
            ...
            if ((plots_count > 3) && (plot_index != 3))
            {
                if (y_min4 != y_max4)
                {
                    ...
                    {
                        y_data4[i]
                            = ((y_data4[i] - y_mn) * (y_max4 - y_min4) / koeff)
                            + y_min4;
                    }
                } else
                {
...
bool Plot::load_recorder_params()
{
    QT_TRY
    {
        ...
        if ((separatedParams.at(4) == "1") && (plots_count > 1))
            ui->cb_use_match->setChecked(true);
...
bool Plot::save_recorder_params()
{
    QT_TRY
    {
        ...
        if (ui->cb_use_match->isChecked())
            params += "1;";
        else
            params += "0;";
        if (pb_grid->isChecked())
            params += "1;";
        else
            params += "0;";
        if (pb_crosshair->isChecked())
            params += "1;";
        else
            params += "0;";
        if (pb_picker_data->isChecked())
            params += "1;";
        else
            params += "0;";
        ...
        foreach (QTableWidgetItem *twi, mktableItemList)
        {
            twi->setData(MKTable::RecorderParamsRole,
                         params + isr_params.at(isr));
            isr++;
        }
    }
    QT_CATCH(...) { return false; }
...
void Plot::set_file_for_write()
{
    QString str = QFileDialog::getSaveFileName(
        this, "Файл", "", "*.*", 0, QFileDialog::DontConfirmOverwrite);
    ...
    if (openFilesList.contains(str))
    {
        QMessageBox::warning(
            this, "Ошибка",
            "Файл занят другим самописцем. Выберите другой файл.");
        return;
    } else
    {
...
void Plot::file_write_start_stop()
{
    ...
    } else
    {
        ...
        QPushButton *rejB
            = modeBox.addButton("Отмена", QMessageBox::RejectRole);
        ...
        if (!file.open(QIODevice::WriteOnly
                       | (open_mode ? QIODevice::Append : QIODevice::Truncate)))
        {
            QMessageBox::critical(
                this, "Ошибка",
                "Не удалось открыть файл для записи. Возможно, он не указан "
                "или у вас нет на это прав.");
            return;
        }
...
qreal Plot::getYcoord(qreal xcoord, int y_data_index)
{
    ...
    {
    case 0:
        y_data_cur = y_data1.get();
        break;
    case 1:
        y_data_cur = y_data2.get();
        break;
    case 2:
        y_data_cur = y_data3.get();
        break;
    case 3:
        y_data_cur = y_data4.get();
        break;
    default:
        y_data_cur = y_data1.get();
    }
...
//###################################################################
PlotISRDialog::PlotISRDialog(QStringList *isr_list, unsigned int isr_index,
                             QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::PlotISRDialog)
{
    ui->setupUi(this);
...
void PlotISRDialog::accept()
{
    ...
    {
        if (ui->le_isr->text().isEmpty())
            (*isr_param_list)[isr_param_index] = "0.0";
        else
            (*isr_param_list)[isr_param_index] = ui->le_isr->text();
    } else
    {
