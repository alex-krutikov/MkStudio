...
void MBMasterXML::load_configuration(QDomDocument &doc)
{
    ...
    for (i = 0; i < module_list.count(); i++)
    {
        ...
        /*
            Console::Print( QString("NODE = %1, SUBNODE = %2  <%3> <%4> <%5>
           <%6>") .arg( mm.node ).arg( mm.subnode ).arg(
           element.attribute("Node") ) .arg(rx.numCaptures() ).arg(rx.cap(1)
           ).arg(rx.cap(2) ) );
        */
        mm.name = element.attribute("Name");
        mm.desc = element.attribute("Desc");
        /*
            Console::Print( QString("  Модуль: %1       Название: %2 Адрес: %3
           Описание: %4\n") .arg(mm.n,2) .arg(mm.name) .arg(mm.node,2)
                           .arg(mm.desc) );
        */
        ...
        for (j = 0; j < slot_list.count(); j++)
        {
            ...
            if (ss.datatype.isRegister())
                ss.addr = element.attribute("Addr").toInt(&ok, 10);
            else
            ...
            ss.module = mm;
            /*
                  Console::Print( QString("    слот:   %1 Адрес: 0x%2 Кол-во: %3
               Тип: %4 Операция: \"%5\" Описание: %6\n") .arg(ss.n,2)
                                 .arg(ss.addr,3,16)
                                 .arg(ss.len,2)
    ...
    Console::Print(Console::Information,
                   "Конфигурации MBMasterPrivateXML загружена.\n");
}
...
void MBMasterXML::saveValues(QDomDocument &doc)
{
    ...
    for (i = 0; i < d->mmslots.size(); i++)
    {
        ...
        {
            if (d->mmslots[i].data[j].status() == MMValue::Ok)
                str += d->mmslots[i].data[j].toString();
            str += ";";
        }
        str.chop(1);
...
/*! \page xml_opros_desc Формат XML документа конфигурации опроса модулей
  ...
  Документ заключается в тег \b MBConfig и состоит из тегов \b Module, в которых
описываются модули.
...
/> <Slot Desc="Частота" Length="64" Type="dwords" N="6" Addr="156" /> <Slot
Desc="Период экс. сглаживания" Length="64" Type="words" N="7" Addr="256" />
    </Module>
  </MBConfig>
\endverbatim
