    ...
    , ui(new Ui::MBMasterWidget)
{
    ...
    ui->tw->verticalHeader()->setDefaultSectionSize(ui->tw->font().pointSize()
                                                    + 11);
...
{
    mbmaster = mm;
    connect(mm.get(), SIGNAL(stateChanged()), SLOT(stateUpdate()),
            Qt::QueuedConnection);
}
...
void MBMasterWidget::stateUpdate()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (j < 0) continue;
        if (j >= m) continue;
        mbmodel->setData(mbmodel->index(i, 0), mbmaster->d->mmslots[j].status,
                         Qt::UserRole);
    }
}
...
void MBMasterWidget::on_tw_doubleClicked(const QModelIndex &index)
{
    ...
    SlotWidget *p = new SlotWidget(this, mbmaster, slot.module.n, slot.n,
                                   mktable_minimize_flag);
    connect(p, SIGNAL(attributes_saved(int, int, QString)), this,
            SIGNAL(attributes_saved(int, int, QString)));
    connect(this, SIGNAL(signalMkTableMinimizeStateChange(bool)), p,
            SLOT(updateMinimizeButtonState(bool)));
    connect(p, SIGNAL(signalMinimize(bool)), this,
            SIGNAL(signalMinimize(bool)));
    connect(p, SIGNAL(signalOstsOpen()), this, SIGNAL(signalOstsOpen()));
    connect(p, SIGNAL(signalOstsClose()), this, SIGNAL(signalOstsClose()));
...
//
//==============================================================================
QVariant MBMasterWidgetTableModel::data(const QModelIndex &index,
                                        int role) const
{
    ...
    case (0): // обычный слот
        ...
        case (Qt::DisplayRole):
            ...
            {
            case (0):
                return table[row].n; // N
            case (1):
                return table[row].addr; // Адрес
            case (2):
                return table[row].len; // Кол-во
            case (3):
                return table[row].datatype.toName(); // Тип
            case (4):
                return table[row].desc; // Описание
            }
            break;
    ...
    case (1): // модуль
        ...
        case (Qt::DisplayRole):
            ...
            case (4):
                str.clear();
                if ((!table[row].name.isEmpty())
                    && (!table[row].desc.isEmpty()))
                    str = " | ";
                return "Модуль " + table[row].name + str
                     + table[row].desc; // Описание
            }
            break;
        case (Qt::BackgroundRole):
            return color_gray;
...
//
//==============================================================================
QVariant MBMasterWidgetTableModel::headerData(int section,
                                              ...
                                              int role) const
{
    ...
    const int len = 5;
    const static char *names[len] = {"N", "Адрес", "Кол-во", "Тип", "Описание"};
    const static int sizes[len] = {30, 50, 50, 80, 80};
...
void MBMasterWidgetTableModel::update_slots(MBMasterXMLPtr mm)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (ss.datatype.isRegister())
            ss.addr = QString::number(mm->d->mmslots[i].addr, 10);
        else
