#include "misc.h"

//=======================================================================================
//
//=======================================================================================
 // Table of CRC values for high–order byte
static const unsigned char auchCRCHi[] = {

} ;

// Table of CRC values for low–order byte
static const unsigned char auchCRCLo[] = {

};

//==============================================================================
//  The function returns the CRC as a unsigned short type
//==============================================================================
WORD get_crc16 (BYTE *ptr, DWORD  DataLen )
{
  BYTE CRCHi = 0xFF;   //high byte of CRC initialized
  BYTE CRCLo = 0xFF;   // low byte of CRC initialized
  unsigned int  Index;          // will index into CRC lookup table
  while(DataLen--)              // pass through message buffer
  {
    Index   = CRCLo ^ (*ptr);      // calculate the CRC
    CRCLo   = CRCHi ^ auchCRCHi[Index];
    CRCHi   = auchCRCLo[Index];
    ptr++;
  }
  return ((CRCHi << 8) | CRCLo) ;
}


//==================================================================================
// CRC32
//==================================================================================

#define CRCPOLY     0xEDB88320UL
/* 1110 1101 1001 1000 1000 0011 0010 0000 */
#define UPDATE_CRC(crc, c) crc = crctable[(BYTE)crc ^ (BYTE)(c)] ^ (crc >> 8)

static DWORD crctable[256];

void make_crc32table()
{
    int   i, j;
    DWORD r;

    for (i = 0; i < 256; i++)
    { r = i;
      for (j = 8; j > 0; j--)
      { if (r & 1) { r = (r >> 1) ^ CRCPOLY; }
        else       { r >>= 1;                }
      }
      crctable[i] = r;
    }
}

DWORD get_crc32( BYTE *ptr, DWORD len)
{
  DWORD crc32;
  crc32=0xFFFFFFFF;
  while ( len )
  { UPDATE_CRC( crc32, *ptr );
    ptr++;
    len--;
  }
  return crc32;
}

DWORD update_crc32( DWORD crc32, BYTE b )
{
  return UPDATE_CRC( crc32, b );
}

//==============================================================================
//
//  Генеpатоp Случайных Чисел (Pавномеpная последовательность)
//  ==========================================================
//
//  value - целое случайное число 0..32767
//
//  Пpимечания:
//  1. После генеpации 32768 чисел последовательность повтоpяется.
//  2. В случайной последовательности из 32768 чисел нет ни одного
//     повтоpяющегося !
//  3. Задаваемое пpи пеpвом вызове число pекомендуется
//     задавать в диапазоне 1..32767, хотя возможно и любое
//     дpугое задание.
//
//==============================================================================
WORD irnd( WORD value )
{ //                       8#31105    8#15415   8#77777
  return ((( (DWORD)value * 0x3245 ) + 0x1B0D) & 0x7FFF );
}
