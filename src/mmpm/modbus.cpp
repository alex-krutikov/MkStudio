...
//==============================================================================
int ModbusMaster::func_43_01_set( BYTE node, BYTE *ptr,
                                      WORD addr, BYTE len, int mode  )
{
  req[0]=node;
  req[1]=func_number;
  req[2] = ( subnode << 4 );
  switch( mode )
  { case(1): req[2]|=0x03; break; // and
    case(2): req[2]|=0x05; break; // or
    case(3): req[2]|=0x07; break; // xor
    default: req[2]|=0x01; break; // set
  }
  req[3]=addr >> 8;
  req[4]=addr;
  req[5]=len;
...
//==============================================================================
int ModbusMaster::func_43_02_set_and(
                                BYTE node, BYTE *ptr, WORD addr, BYTE len  )
{
...
//==============================================================================
int ModbusMaster::func_43_03_set_or(
                                BYTE node, BYTE *ptr, WORD addr, BYTE len  )
{
...
//==============================================================================
int ModbusMaster::func_43_04_set_xor(
                                BYTE node, BYTE *ptr, WORD addr, BYTE len  )
{
...
int ModbusMaster::func_45_01_reset_ans( BYTE node, DWORD code  )
{
  ...
  if( !go() ) return 0;
  if( ans_len != 9) return 0;
  for( i=0; i<9; i++ ) if( req[i]!=ans[i] ) return 0;
  return 1;
}
...
//==============================================================================
int ModbusMaster::func_45_04_flash_read(
                                  BYTE node, DWORD addr, BYTE len, BYTE*ptr )
{
  req[0]=node;
  req[1]=0x45;
...
//==============================================================================
int ModbusMaster::func_45_05_flash_erase(
               BYTE node, DWORD sector_begin, DWORD sector_end)
{
  int i;
  ...
  for( i=0; i<13; i++ ) if( req[i]!=ans[i] ) return 0;
  return 1;
}
...
//==============================================================================
int ModbusMaster::func_45_06_buffer_write(
                           BYTE node, BYTE *ptr, WORD addr, BYTE len )
{
  int i;

//Console::Print( tr("<буфер ptr=%1 addr=%2 len=%3> ").arg((int)ptr,  8,16,QChar('0') )
...
//                                                 .arg(len,       8,16,QChar('0')  ));
  ...
  if( ans[3] != len ) return 0;
  for( i=0; i<len; i++ ) if( req[i+6]!=ans[i+4] ) return 0;
  return 1;
}
...
//Console::Print( tr("<пишем сектор addr=%1 len=%2> ").arg(addr, 8,16,QChar('0') )
//                                                 .arg(len,  8,16,QChar('0')  ));
