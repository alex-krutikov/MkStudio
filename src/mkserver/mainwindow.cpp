...
public:
    ...
    int query(const QByteArray &req, QByteArray &ans, int *errorcode = 0)
    {
        ...
        if (CRC::CRC16(req))
        {
            Console::Print(Console::ModbusError, "Request packet: CRC ERROR. [ "
                                                     + QByteArray2QString(req)
                                                     + " ]\n");
            return 0;
        }
...
MainWindow::MainWindow()
{
    ...
    connect(cb_portname, SIGNAL(currentIndexChanged(int)), this,
            SLOT(settingsChanged()));
    connect(cb_portspeed, SIGNAL(currentIndexChanged(int)), this,
            SLOT(settingsChanged()));
    connect(le_timeout, SIGNAL(textChanged(const QString &)), this,
            SLOT(settingsChanged()));
    connect(cb_protocol, SIGNAL(currentIndexChanged(int)), this,
            SLOT(settingsChanged()));
    connect(le_tcp_port, SIGNAL(textChanged(const QString &)), this,
            SLOT(settingsChanged()));
    connect(cb_mb_packet, SIGNAL(toggled(bool)), this, SLOT(settingsChanged()));
    connect(pb_console_clear, SIGNAL(clicked()), te, SLOT(clear()));
    connect(sb_repaly_delay, SIGNAL(valueChanged(int)), this,
            SLOT(replyDelayChanged(int)));
...
void MainWindow::settingsChanged()
{
    ...
    QString portname
        = cb_portname->itemData(cb_portname->currentIndex()).toString();
...
{
    if (cb_console_pause->isChecked()) return;
