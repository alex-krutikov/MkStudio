...
static void print_help()
{
    QString str
        = "\n"
          ...
          "    -fast\n"
          "    -daysbefore=[Number for set synchronize period on days: days "
          "before today is start, today is end]\n"
          "    -monthsbefore=[Number for set synchronize period on month: "
          "months before current month is start, current month is end]\n"
...
{
    return QDateTime::currentDateTime().toString("ddMMyyhhmmss") + " "
         + hostportnode;
}
//=============================================================================
// Инициализация
...
static int init()
{
    ...
    foreach (QString str, command_arguments)
    {
        if (str.startsWith("-host="))
        {
            ...
            if (hostportnode.startsWith(" "))
                hostportnode = hostportnode.right(hostportnode.length() - 1);
    ...
    {
        if (node < 10)
            xmlf = "/mksync_00" + QString::number(node) + ".xml";
        else if (node < 100)
            xmlf = "/mksync_0" + QString::number(node) + ".xml";
        else
            xmlf = "/mksync_" + QString::number(node) + ".xml";
        QDomDocument doc;
        QDomNodeList list;
        QDomElement element;
        ...
        for (int i = 0; i < list.count(); i++)
        {
            ...
            dt_files.insert(element.attribute("file", "0"),
                            element.attribute("date_time", "0"));
        }
        xmlfile.close();
    }
...
bool sync_file(const QString filename, const int filesize, const QString &path)
{
    ...
    if (dtmode)
    {
        ...
        {
            dt_files.insert(remote_path + path + filename,
                            xfi.mtime.addDays(1).toString("ddMMyyhhmmss"));
        }
        ...
        if (hashDT == xfi.mtime.toString("ddMMyyhhmmss") && filecheck.exists())
            return true;
    ...
    while (rep--)
    {
        res = xf->openFile(remote_path + path + filename,
                           XFiles::FA_OPEN_EXISTING, &id);
        xfile_print_error(res);
        if (res == XFiles::Ok) break;
    }
    if (res != XFiles::Ok) return false;
...
int main(int argc, char *argv[])
{
    ...
    if (node < 10)
        lockName = "mksync_00" + QString::number(node);
    else if (node < 100)
        lockName = "mksync_0" + QString::number(node);
    else
        lockName = "mksync_" + QString::number(node);
