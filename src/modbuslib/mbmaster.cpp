...
void MBMasterPrivate::polling_start()
{
    ...
    for (i = 0; i < mmslots.count(); i++)
    {
        ...
        { // classic modbus registers
            ...
            { // extended modbus registers
                ...
                int regs_per_packet
                    = ((int)((max_packet_length & 0x7FFFFFFF) - 5)) / 4;
                k = 0;
                while (k < ps.len)
                {
            ...
            { // standart modbus registers
                ...
                case (MBDataType::HoldingRegisters):
                    regs_per_packet
                        = ((int)((max_packet_length & 0x7FFFFFFF) - 5)) / 2;
                    break;
                case (MBDataType::DiscreteInputs):
                case (MBDataType::Coils):
                    regs_per_packet
                        = ((int)((max_packet_length & 0x7FFFFFFF) - 5)) * 8;
                    break;
                default:
                    continue;
                    break;
    ...
    for (i = 0; i < mmslots.count(); i++)
    {
        ...
        for (k = 0; k < ps.len; k++)
        {
            ...
            {
                if (ps.datatype.id() == MBDataType::BitsArduino)
                {
                    a = k / 8 + ps.addr;
                } else
                {
            ...
            {
                if (ps.datatype.id() == MBDataType::Bits)
                {
                    a = k / 8 + ps.addr;
                } else
                {
            ...
            transactions_write_map.insert(ModuleSlotIndex(ps.module.n, ps.n, k),
                                          m);
            m++;
        }
    }
    // for( i=0; i<transactions_write.count(); i++ )
    //{ Console::Print( Console::Debug, "запись:" + QByteArray2QString(
    // transactions_write[i].request ) + "\n"  );
    //}
...
//
//===================================================================
void MBMasterPrivate::add_slot(int module_index, int slot_index, int addr,
                               int len, MBDataType datatype,
                               const QString &description)
{
    int i;
    MMSlot slot;
...
void MBMasterPrivate::run()
{
    ...
    while (!thread_exit_flag) // цикл опроса
    {
        if ((full_time > 100) && (data_request_timer.elapsed() > 3 * full_time))
        {
            ...
            for (k = 0; k < nk; k++)
            {
                if (!mmslots[k].data_read_flag)
                    mmslots[k].status = MMSlot::Skiped;
            ...
            for (j = 0; j < transactions_write.count(); j++)
            {
                ...
                { // ОШИБКА
                    ...
                    { // отменяем все транзакции данного слота
                        ...
                        {
                            if ((transactions_write[k].slot->n
                                 == transactions_write[j].slot->n)
                                && (transactions_write[k].slot->module.n
                                    == transactions_write[j].slot->module.n))
                            {
...
void MBMasterPrivate::optimize_write()
{
    ...
    for (i = 0; i < (ni - 1); i++)
    {
        ...
        } else
        {
            ...
            { // запись по AND,OR,XOR
                if ((addr1 != addr2) && (addr1 + len1) != addr2)
                    goto skip; // ад
            }
            flags_array[i] = k;
            flags_array[i + 1] = k;
    ...
    for (i = 0; i < ni; i++)
    {
        if (flags_array[i] == 0) continue;
        {
            k = flags_array[i];
            j = i;
            while ((j < ni) && (flags_array[j] == k))
                j++;
            j--;
            // Console::Print( QString("ОПТИМИЗИРОВАННАЯ ЗАПИСЬ [%1;%2]
            // (%3)\n").arg(i).arg(j).arg(k) );
...
void MBMasterPrivate::optimize_write_holding()
{
    ...
    {
        if (!transactions_write[i].execute_flag) goto skip;
        if (transactions_write[i].slot->datatype.id()
            != MBDataType::HoldingRegisters)
            goto skip;
        if (!transactions_write[i + 1].execute_flag)
        {
            goto skip;
    ...
    for (i = 0; i < ni; i++)
    {
        if (flags_array[i] == 0) continue;
        {
            ...
            while ((j < ni) && (flags_array[j] == k))
                j++;
            j--;
            // Console::Print( Console::Debug, QString("ОПТИМИЗИРОВАННАЯ ЗАПИСЬ
            // [%1;%2] (%3)\n").arg(i).arg(j).arg(k) );
...
void MBMasterPrivate::optimize_write_coils()
{
    ...
    {
        if (!transactions_write[i].execute_flag) goto skip;
        if (transactions_write[i].slot->datatype.id() != MBDataType::Coils)
            goto skip;
        if (!transactions_write[i + 1].execute_flag)
        {
            goto skip;
...
bool MBMasterPrivate::process_transaction(MMSlotTransaction &tr)
{
    ...
    {
        if (auto_skip_mode && !tr.slot->data_read_flag) { goto skiped; }
    }
...

    {
        ...
        const QByteArray request
            = isArduino ? encodeArduinoTransport(tr.request) : tr.request;
        if (isArduino)
        {
...

    {
        ...
        if (tr.slot->datatype.isRegister())
        {
            ...
            case (0x02): // Read Discrete Inputs
                if (tr.slot->datatype.id() != MBDataType::DiscreteInputs)
                    goto exit;
                for (i = 0; i < tr.length; i++)
                {
                    tr.slot->data[i + tr.offset]
            ...
            case (0x03): // Read Holding Registers
                ...
                case (MBDataType::HoldingRegisters):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = swap_short(
                            *((short *)(tr.answer.data() + 3 + 2 * i)));
                    }
                    break;
                case (MBDataType::FloatsHoldingRegHiLo):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = ptr2float(
                            tr.answer.constData() + 3 + 4 * i, false);
                    }
                    break;
                case (MBDataType::FloatsHoldingRegLoHi):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = ptr2float(
                            tr.answer.constData() + 3 + 4 * i, true);
                    }
                    break;
                case (MBDataType::DwordsHoldingRegHiLo):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = ptr2dword(
                            tr.answer.constData() + 3 + 4 * i, false);
                    }
                    break;
                case (MBDataType::DwordsHoldingRegLoHi):
        ...
        } else if (tr.slot->datatype.isArduino())
        {
            ...
            { // чтение
                ...
                case (MBDataType::BitsArduino):
                    ...
                    {
                        tr.slot->data[i + tr.offset]
                            = ((*((char *)(tr.answer.data() + data_offset
                                           + i / 8)))
                               & (1 << (i & 7)))
                                ? (1)
                                : (0);
                ...
                case (MBDataType::BytesArduino):
                    ...
                    {
                        tr.slot->data[i + tr.offset]
                            = *((char *)(tr.answer.data() + data_offset + i));
                    }
                    break;
                case (MBDataType::WordsArduino):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = *(
                            (short *)(tr.answer.data() + data_offset + 2 * i));
                    }
                    break;
                case (MBDataType::DwordsArduino):
                    for (i = 0; i < tr.length; i++)
                    {
                        tr.slot->data[i + tr.offset] = *(
                            (int *)(tr.answer.data() + data_offset + 4 * i));
                    }
                    break;
                case (MBDataType::FloatsArduino):
                    for (i = 0; i < tr.length; i++)
        ...
        {
            if ((tr.request[1] != (char)0x41) && (tr.request[1] != (char)0x43))
                goto exit; // check operation code
            if (tr.answer[0] != tr.request[0]) goto exit;
            if (tr.answer[1] != tr.request[1]) goto exit;
            if (tr.answer[2] != tr.request[2]) goto exit;
            if (CRC::CRC16(tr.answer)) goto exit;
...
//
//===================================================================
void MBMasterPrivate::setSlotValue(int module, int slot, int index,
                                   const MMValue &value)
{
    union
    {
    ...
    int i, j;
    // Console::Print( QString("Запись: %1/%2/%3
    ...
    if ((i >= 0) && (i < transactions_write.count()))
    {
        ...
        if (datatype.isRegister())
        {
            ...
            case (MBDataType::DwordsHoldingRegHiLo):
                ...
                // Console::Print( Console::Debug, "запись :" +
                // QByteArray2QString( transactions_write[i].request ) + "\n" );
                buff_len = 4;
                break;
            case (MBDataType::DwordsHoldingRegLoHi):
            ...
            for (j = 0; j < buff_len; j++)
                transactions_write[i].request.append(buff.buff[j]);
            CRC::appendCRC16(transactions_write[i].request);
        } else if (datatype.isArduino())
        {
            transactions_write[i].request.resize(4);
            // Console::Print(Console::Debug, "запись 1:" + QByteArray2QString(
            // transactions_write[i].request ) + "\n"  );
            switch (transactions_write[i].slot->datatype.id())
            {
            case (MBDataType::BitsArduino):
                ...
                break;
            case (MBDataType::BytesArduino):
                buff.i8 = (value.toInt() & 0xFF);
                buff_len = 1;
                break;
            case (MBDataType::WordsArduino):
                buff.i16 = (value.toInt() & 0xFFFF);
                buff_len = 2;
                break;
            case (MBDataType::DwordsArduino):
                buff.i32 = value.toInt();
                buff_len = 4;
                break;
            case (MBDataType::FloatsArduino):
            ...
            case (MBDataType::Bits):
                if (value.toInt() != 0)
                {
                    ...
                    transactions_write[i].request[2]
                        = 0x05
                        | (transactions_write[i].slot->module.subnode
                           << 4); // OR
                } else
                {
                    buff.i8 = 0xFF & (~(1 << (index & 7)));
        ...
        // Console::Print( Console::Debug, "запись 2:" + QByteArray2QString(
        // transactions_write[i].request ) + "\n"  );
    }
}
...
//
//===================================================================
void MBMasterPrivate::setSlotAttributes(int module, int slot,
...
MBMasterPrivate::decodeModuleDefinition(const QByteArray &ba)
{
...
#include "packed_struct_end.h"
    ...
    md.releaseDate
        = QString::fromLatin1(st->ReleaseDate, sizeof(st->ReleaseDate));
    md.releaseMadeBy
        = QString::fromLatin1(st->ReleaseMadeBy, sizeof(st->ReleaseMadeBy));
    md.productName
        = QString::fromLatin1(st->ProductName, sizeof(st->ProductName));
    md.productDescription = QString::fromLatin1(st->ProductDescription,
                                                sizeof(st->ProductDescription));
    md.companyName
        = QString::fromLatin1(st->CompanyName, sizeof(st->CompanyName));
    md.legalCopyright
        = QString::fromLatin1(st->LegalCopyright, sizeof(st->LegalCopyright));
    return md;
}
...
{
    d->add_module(module_index, node, subnode, name, description);
}
...
{
    d->add_slot(module_index, slot_index, addr, len, datatype, description);
}
...
*/
//===================================================================
void MBMaster::setSlotAttributes(int module, int slot,
                                 const QString &attributes)
{
