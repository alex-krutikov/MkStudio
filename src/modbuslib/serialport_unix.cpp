...
bool SerialPortPrivate::open()
{
    ...
    if (tcgetattr(fd, &ts)) { return false; }
    cfmakeraw(&ts);
    cfsetispeed(&ts, br);
    cfsetospeed(&ts, br);
    ts.c_oflag |= CLOCAL;
    ...
    ts.c_cc[VMIN] = 0;
    if (tcsetattr(fd, TCSANOW, &ts)) { return false; }
    return true;
}
                             ...
                             int *errorcode)
{
    ...
    while (1)
    {
        ...
        if (ret < 0)
        {
            Console::Print(Console::Error,
                           "MODBUS: ERROR: Can't write to port.\n");
            return 0;
        }
        i += ret;
    ...
    while (1)
    {
        ...
        {
            Console::Print(Console::Error,
                           "MODBUS: ERROR: Can't read from port.\n");
            return 0;
        }
        i += ret;
    ...
    if (i != answer_size)
    {
        Console::Print(
            Console::Error,
            QString(
                "MODBUS: ERROR: Wrong answer length. (expected=%1, real=%2) ")
                .arg(answer_size)
                .arg(i));
        QByteArray ba = answer;
        ba.resize(i);
        Console::Print(Console::Error,
                       " Answer: " + QByteArray2QString(ba) + "\n");
    }
                                 ...
                                 int *errorcode, int xbee_addr)
{
    ...
    while (1)
    {
        ...
        {
            Console::Print(Console::Error,
                           "XBEE: ERROR: Can't write to port.\n");
            return 0;
        }
        i += ret;
        if (i == ba.size()) break;
    ...
    while (1)
    {
        ...
        {
            Console::Print(Console::Error,
                           "XBEE: ERROR: Can't read from port.\n");
            return 0;
        }
        i += ret;
        if (i == ba.size()) break;
...
QStringList SerialPortPrivate::queryComPorts()
{
    ...
    foreach (QString str, dir.entryList(QDir::System, QDir::Name))
    {
        ...
        if (fd < 0) continue;
        if (tcgetattr(fd, &ts) == 0) { sl << str + ";Serial Port"; }
        ::close(fd);
    }
    return sl;
}
