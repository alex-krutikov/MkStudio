...
//
//=============================================================================
int MbUdpPort::query(const QByteArray &request, QByteArray &answer,
                     int *errorcode)
{
    Q_UNUSED(errorcode)
    ...
    if (!d->socket) { d->socket.reset(new QUdpSocket); }
    ...
    Console::Print(Console::ModbusPacket,
                   "MB_UDP: udp_header  = " + QByteArray2QString(udp_header)
                       + "\n");
    Console::Print(Console::ModbusPacket,
                   "MB_UDP: udp_request = " + QByteArray2QString(udp_req)
                       + "\n");
    ...
    while (timeout_timer.elapsed() < udp_timeout)
    {
        ...
        if (d->socket->hasPendingDatagrams())
        {
            ...
            if (!datagram.isValid())
            {
                Console::Print(
                    Console::Error,
                    "MB_UDP: Error: Received UDP datagram is invalid.\n");
                return 0;
            }
            ...
            if (udp_ans.size() < 6)
                ok = false;
            else if (udp_ans[0] != udp_header[0])
                ok = false;
            else if (udp_ans[1] != udp_header[1])
                ok = false;
            else if (udp_ans[2] || udp_ans[3])
                ok = false;
            else if ((((uint8_t)udp_ans[4] << 8) | (uint8_t)udp_ans[5])
                     != (udp_ans.size() - 6))
                ok = false;
            else if ((udp_ans.size()) != (answer.size() + 4))
                ok = false; // Modbus UDP packet length check

            if (!ok)
            {
                Console::Print(
                    Console::Error,
                    QString("MB_UDP: Error: Wrong UDP. Request: [%1] Answer: "
                            "[%2]\n")
                        .arg(QByteArray2QString(udp_header + udp_req))
                        .arg(QByteArray2QString(udp_ans)));
            }
    ...
    Console::Print(Console::ModbusPacket,
                   "MB_UDP:  udp_answer = " + QByteArray2QString(udp_ans)
                       + "\n");
    ...
    Console::Print(Console::ModbusPacket,
                   "MB_UDP:   mb_answer = " + QByteArray2QString(answer.left(j))
                       + "\n");
    return j;
}
