...
//
//=============================================================================
int MbTcpPort::query(const QByteArray &request, QByteArray &answer,
                     int *errorcode)
{
    ...
    if (!d->socket)
    {
        ...
        if (!ok)
        {
            Console::Print(Console::Error,
                           "MB_TCP: Error: Unable to connect to host `"
                               + d->portname + "`.\n");
        }
    }
    ...
    Console::Print(Console::ModbusPacket,
                   "MB_TCP: tcp_header  = " + QByteArray2QString(tcp_header)
                       + "\n");
    Console::Print(Console::ModbusPacket,
                   "MB_TCP: tcp_request = " + QByteArray2QString(tcp_req)
                       + "\n");
    ...
    while (1)
    {
        if (d->socket->state() != QAbstractSocket::ConnectedState)
        {
            open();
            Console::Print(Console::Error,
                           "MB_TCP: Error: Socket state is not `Connected`\n");
            return 0;
        }
    ...
    Console::Print(Console::ModbusPacket,
                   "MB_TCP:  tcp_answer = " + QByteArray2QString(tcp_ans)
                       + "\n");
    ...
    answer = tcp_ans;
    Console::Print(Console::ModbusPacket,
                   "MB_TCP:   mb_answer = " + QByteArray2QString(answer.left(j))
                       + "\n");
    return j;
error:
    Console::Print(Console::Error, "MB_TCP: Error: Wrong tcp anser: "
                                       + QByteArray2QString(tcp_ans) + "\n");
    close();
    return 0;
error2:
