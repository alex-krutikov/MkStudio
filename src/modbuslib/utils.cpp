...
void ProtocolAnalyser::setRequest(const QByteArray &request)
{
    ...
    if ((request[1] == 0x41) || ( request[1] == 0x43))
    {
        if (request[2] & 0x08)
        {
            ...
            case( 0x00 ): // read
                ...
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 READ Addr=%1 DataSize=%2 \n").arg(intToHex(d->dataAddr, 8)).arg(d->dataSize));
                break;
            case( 0x01 ): // set
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON32_WRITE_SET;
                d->responseLength = d->dataSize + 11;
                data = request.mid(9, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 WRITE(SET) Addr=%1 DataSize=%2 Data=%3\n").arg(intToHex(d->dataAddr, 8)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x03 ): // and
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON32_WRITE_AND;
                d->responseLength = d->dataSize + 11;
                data = request.mid(9, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 WRITE(AND) Addr=%1 DataSize=%2 Data=%3\n").arg(intToHex(d->dataAddr, 8)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x05 ): // or
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON32_WRITE_OR;
                d->responseLength = d->dataSize + 11;
                data = request.mid(9, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 WRITE(OR) Addr=%1 DataSize=%2 Data=%3\n").arg(intToHex(d->dataAddr, 8)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x07 ): // xor
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON32_WRITE_XOR;
                d->responseLength = d->dataSize + 11;
                data = request.mid(9, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 WRITE(XOR) Addr=%1 DataSize=%2 Data=%3\n").arg(intToHex(d->dataAddr, 8)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            }
            ...
            // MIKKON
            d->dataAddr = (static_cast<uint8_t>(request[3]) << 8) | static_cast<uint8_t>(request[4]);
            ...
            case( 0x00 ): // read
                ...
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON READ Addr=%1 DataSize=%2 \n").arg(intToHex(d->dataAddr, 4)).arg(d->dataSize));
                break;
            case( 0x01 ): // set
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON_WRITE_SET;
                d->responseLength = d->dataSize + 8;
                data = request.mid(6, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON WRITE(SET) Addr=%1 DataSize=%2 Data=[%3]\n").arg(intToHex(d->dataAddr, 4)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x03 ): // and
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON_WRITE_AND;
                d->responseLength = d->dataSize + 8;
                data = request.mid(6, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON WRITE(AND) Addr=%1 DataSize=%2 Data=[%3]\n").arg(intToHex(d->dataAddr, 4)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x05 ): // or
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON_WRITE_OR;
                d->responseLength = d->dataSize + 8;
                data = request.mid(6, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON WRITE(OR) Addr=%1 DataSize=%2 Data=[%3]\n").arg(intToHex(d->dataAddr, 4)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            case( 0x07 ): // xor
                d->protocol = ProtocolAnalyserPrivate::Protocol::MIKKON_WRITE_XOR;
                d->responseLength = d->dataSize + 8;
                data = request.mid(6, d->dataSize);
                Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON WRITE(XOR) Addr=%1 DataSize=%2 Data=[%3]\n").arg(intToHex(d->dataAddr, 4)).arg(d->dataSize).arg(QByteArray2QString(data)));
                break;
            }
        }
    }
    ...
    {
        Console::Print( Console::ModbusPacket, "Protocol: MIKKON function 0x45\n");
...
void ProtocolAnalyser::setResponse(const QByteArray &response)
{
    ...
    case ProtocolAnalyserPrivate::Protocol::MIKKON_READ:
        ...
        if (size != d->dataSize)
        {
            Console::Print( Console::ModbusPacket, QString("Protocol: ERROR in response: data size=%1 expected value=%2\n").arg(size).arg(d->dataSize));
        }
        data = response.mid(4, size);
        Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON READ Data=[%1]\n").arg(QByteArray2QString(data)));
        break;
    ...
    case ProtocolAnalyserPrivate::Protocol::MIKKON_WRITE_XOR:
        addr = (static_cast<uint8_t>(response[3]) << 8) | static_cast<uint8_t>(response[4]);;
        size = static_cast<uint8_t>(response[5]);
        if (addr != d->dataAddr)
        {
            Console::Print( Console::ModbusPacket, QString("Protocol: ERROR in response: data addr=%1 expected value=%2\n").arg(intToHex(addr, 4)).arg(intToHex(d->dataAddr, 4)));
        }
        if (size != d->dataSize)
        {
            Console::Print( Console::ModbusPacket, QString("Protocol: ERROR in response: data size=%1 expected value=%2\n").arg(size).arg(d->dataSize));
        ...
        Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON WRITE Data=[%1]\n").arg(QByteArray2QString(data)));
        break;
    ...
    case ProtocolAnalyserPrivate::Protocol::MIKKON32_READ:
        ...
        Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 READ Data=[%1]\n").arg(QByteArray2QString(data)));
        break;
    ...
    case ProtocolAnalyserPrivate::Protocol::MIKKON32_WRITE_XOR:
        ...
        if (addr != d->dataAddr)
        {
            Console::Print( Console::ModbusPacket, QString("Protocol: ERROR in response: data addr=%1 expected value=%2\n").arg(intToHex(addr, 8)).arg(intToHex(d->dataAddr, 8)));
        }
        if (size != d->dataSize)
        {
            Console::Print( Console::ModbusPacket, QString("Protocol: ERROR in response: data size=%1 expected value=%2\n").arg(size).arg(d->dataSize));
        ...
        Console::Print( Console::ModbusPacket, QString("Protocol: MIKKON32 WRITE Data=[%1]\n").arg(QByteArray2QString(data)));
        break;
    default:
        break;
    }
