...
int main(int argc, char *argv[])
{
    ...
    if (initdialog->portname().isEmpty())
    {
        QMessageBox::critical(
            0, app_header,
            "В вашей системе не обнаружено последовательных портов.\n\n"
    ...
    } else
    {
        ...
        if (ret2 != QDialog::Accepted) { return 0; }
    ...
    if (ret2 == QDialog::Accepted)
    {
        ...
        if (initdialog->filename().contains(".xml"))
        {
            ...
            MainWindowXml mainwindow(0, initdialog->portname(),
                                     initdialog->portspeed(),
                                     initdialog->host(), initdialog->filename(),
                                     initdialog->int_module_node(),
                                     initdialog->int_module_subnode());
            mainwindow.setWindowTitle(initdialog->module_name() + "  - "
                                      + app_header);
            mainwindow.show();
            delete initdialog;
            initdialog = 0;
