    ...
    , arduinoOnly(isArduinoOnly)
{
    ...
    current_config_values_filename
        = settings.value("values_file", "").toString();
    move(pos);
    resize(size);
    ...
    connect(shortcutDel, SIGNAL(activated()), this, SLOT(del_key_pressed()));

    QShortcut *shortcutCopyToConsole
        = new QShortcut(QKeySequence(Qt::Key_F5), this);
    connect(shortcutCopyToConsole, SIGNAL(activated()), this,
            SLOT(copy_to_console()));
    ...
    connect(tw->horizontalHeader(), SIGNAL(sectionDoubleClicked(int)),
            SLOT(action_tw_header_double_clicked(int)));
    connect(tw, SIGNAL(signalMinimizeStateChange(bool)), mbmasterwidget,
            SLOT(slotMkTableMinimizeStateChange(bool)));
    connect(tw, SIGNAL(signalMinimizeAllHide(bool)), mbmasterwidget,
            SLOT(slotMkTableMinimizeAllHide(bool)));
    connect(mbmasterwidget, SIGNAL(signalMinimize(bool)), tw,
            SLOT(slotMinimize(bool)));
    connect(mbmasterwidget, SIGNAL(signalOstsOpen()), tw, SLOT(slotPlotOpen()));
    connect(mbmasterwidget, SIGNAL(signalOstsClose()), tw,
            SLOT(slotPlotClose()));
    ...
    play_mode = false;
    connect(tw, SIGNAL(customContextMenuRequested(const QPoint &)), this,
            SLOT(context_menu()));
    tw->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(tw, SIGNAL(item_changed_by_editor()), this,
            SLOT(make_undo_point()));
...
{
    bool ok;
    int i = QInputDialog::getInt(this, Utils::AppInfo::title(),
                                 "Количество строк:", tw->rowCount(), 0, 1000,
                                 1, &ok);
...
void MainWindow::on_action_tw_columns_triggered()
{
    bool ok;
    int i = QInputDialog::getInt(this, Utils::AppInfo::title(),
                                 "Количество столбцов:", tw->columnCount(), 0,
                                 1000, 1, &ok);
...
bool MainWindow::load_conf(const QString &filename)
{
    ...
    {
        QMessageBox::information(this, Utils::AppInfo::title(),
...
void MainWindow::on_action_load_triggered()
{
    QString filename = QFileDialog::getOpenFileName(
        this, "Загрузить", current_config_filename, "Конфигурация (*.xml)");
...
void MainWindow::on_action_save_as_triggered()
{
    QString fileName = QFileDialog::getSaveFileName(this, "Сохранить", "",
                                                    "Конфигурация (*.xml)");
    if (!fileName.isEmpty()) { save_conf(fileName); }
}
...
void MainWindow::on_action_link_triggered()
{
    ...
    {
        posAddress
            = QString("(%1;%2)").arg(titem->column() + 1).arg(titem->row() + 1);
    } else
    {
        posAddress = "(-;-)";
    }
    ...
    if (topRow == bottomRow) { dialog.cbox_fill_row->setEnabled(false); }
    ...
    for (i = topRow; i <= bottomRow; i++)
    {
        for (j = leftColumn; j <= rightColumn; j++)
        {
            ...
            {
                module_titem += (i - topRow) * flag_array[0][2]
                              + (j - leftColumn) * flag_array[1][2];
                slot_titem += (i - topRow) * flag_array[0][1]
                            + (j - leftColumn) * flag_array[1][1];
                n_titem += (i - topRow) * flag_array[0][0]
                         + (j - leftColumn) * flag_array[1][0];
            } else // только строки или только столбцы
            {
                int k = 0;
                if (b[0])
                    k = (bottomRow - topRow + 1) * (j - leftColumn) + i
                      - topRow; // строки
                if (b[1])
                    k = (i - topRow) * (rightColumn - leftColumn + 1) + j
                      - leftColumn; // столбцы
...
void MainWindow::fromClipboard()
{
    ...
    const QByteArray ba = QApplication::clipboard()->mimeData()->data(
        "application/mkstudio-cells-mime");
    QDataStream ds(ba);
...
//==============================================================================
/// Пункт меню "Вырезать" (копировать в буфер обмена и очистить выделенные
/// ячейки)
//==============================================================================
...
void MainWindow::make_undo_point()
{
    ...
    if (undo_list.count() > 20) { undo_list.removeFirst(); }
...
void MainWindow::on_action_redo_triggered()
{
    ...
    undo_list_current_point--;
    applay_undo_point();
    if (undo_list_current_point == 0) { action_redo->setEnabled(false); }
    action_undo->setEnabled(true);
}
...
void MainWindow::applay_undo_point()
{
    ...
    tw->clear();
    QByteArray ba
        = undo_list.at(undo_list.size() - undo_list_current_point - 1);
    QDataStream ds(ba);
    QTableWidgetItem titem;
...
void MainWindow::on_action_values_save_triggered()
{
    ...
    fileName = QFileDialog::getSaveFileName(
        this, "Сохранить", fileName + ".values.xml", "Значения (*.values.xml)");
...
void MainWindow::on_action_values_load_triggered()
{
    ...
    fileName = QFileDialog::getOpenFileName(this, "Загрузить", fileName,
                                            "Значения (*.values.xml)");
...
void MainWindow::on_action_settings_triggered()
{
    ...
    dialog.cb_mbconsole_packets->setChecked(Console::messageTypes()
...
void MainWindow::on_tw_itemSelectionChanged()
{
    ...
    if (c != 1) return;
    if (si.at(0) == 0) return;
    statusbar->showMessage("Связка: "
                           + si.at(0)->data(MKTable::AssignRole).toString());
}
...
void MainWindow::timerEvent(QTimerEvent *event)
{
    ...
    full_time->setText(
        QString(" Время цикла: %1 мс ").arg(mbmaster->full_time()));
