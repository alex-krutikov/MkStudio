...
QVariant Model::data ( const QModelIndex & index, int role ) const
{
  ...
  { case( Qt::EditRole ):
      ...
      { Item &item = items[row];
        ...
        { case(0): // адрес
          ...
          case(5):
            ...
            break;
          case(6): return item.desc;
        }
      }
      break;

    case( Qt::FontRole ):
      if (items[row].reg_num_is_big_flag && column == 1) return itemfont_italic;
    ...
    case( Qt::ForegroundRole ):
      ...
      if( items[row].error_flag ) return QColor("darkblue");
      else if( column==0 && items[row].align_flag ) return QColor("red");
      break;
    ...
    case( Qt::TextAlignmentRole ):
      ...
      { case(0): return (int) Qt::AlignVCenter|Qt::AlignRight ;
        case(1): return (int) Qt::AlignVCenter|Qt::AlignHCenter;
        case(2): return (int) Qt::AlignVCenter|Qt::AlignHCenter;
        case(3): return (int) Qt::AlignVCenter|Qt::AlignLeft;
        case(4): return (int) Qt::AlignVCenter|Qt::AlignHCenter;
        case(5): return (int) Qt::AlignVCenter|Qt::AlignHCenter;
        case(6): return (int) Qt::AlignVCenter|Qt::AlignLeft;
      }
      break;
  }
...
Qt::ItemFlags Model::flags ( const QModelIndex & index ) const
{
  ...
  if (index.column() == 1) return Qt::ItemIsEnabled|Qt::ItemIsSelectable; //column with register number selected
  if (index.column() == 2) return Qt::ItemIsEnabled|Qt::ItemIsSelectable; //column with register type selected
...
bool Model::setData( const QModelIndex & index, const QVariant & value, int role)
{
  ...
  {
    case (Qt::UserRole): {
      if (items[row].addr < 0) break;  //no register type if there's no address
...
void Model::recalc_items()
{
  ...
  for(i=0;i<n;i++)
  {
    ...
    if(      item.size == 2 ) { if((a-a0)&1) item.align_flag = true; }
    else if( item.size == 3 ) { item.align_flag = true; }
    else if( item.size >= 4 ) { if((a-a0)&3) item.align_flag = true; }
...
{
  while( undo_buffer_pointer-- ) undo_buffer.removeFirst();
                          ...
                          const QModelIndex & index ) const
{
  ...
  { Qt::Alignment a = Qt::AlignVCenter|Qt::AlignHCenter;
    switch( index.column() )
    { case(0): a = Qt::AlignVCenter|Qt::AlignHCenter; break;
      case(1): a = Qt::AlignVCenter|Qt::AlignLeft;    break;
      case(2): a = Qt::AlignVCenter|Qt::AlignLeft;    break;
      case(3): a = Qt::AlignVCenter|Qt::AlignLeft;    break;
      case(4): a = Qt::AlignVCenter|Qt::AlignHCenter; break;
      case(5): a = Qt::AlignVCenter|Qt::AlignHCenter; break;
      case(6): a = Qt::AlignVCenter|Qt::AlignLeft;    break;
    }
    le->setAlignment( a );
