...
MainWindow::MainWindow()
{
  ...
  connect( recent_files_group, SIGNAL(  triggered(QAction*) ) ,
           this,               SLOT( recent_files_activated(QAction*) ) );
  ...
  connect( tw, SIGNAL( activated(const QModelIndex&) ), tw, SLOT( edit(const QModelIndex&) ) );
  tw->setColumnHidden(1, !action_show_modbus->isChecked());
  tw->setColumnHidden(2, !action_show_modbus->isChecked());

  file_load();

  connect(tw, SIGNAL(activated(const QModelIndex&)),this, SLOT(check_tw_activation(const QModelIndex&))); //WO!
  update_recent_files();
}
...
void MainWindow::file_save()
{
  ...
  {  QDomElement row = doc.createElement("Row");
     ...
     if( items[i].rnum >= 0 )       row.setAttribute( "Register_number",  items[i].rnum );  //wo
     if (items[i].rtype.getBody() != None) row.setAttribute( "Register_type",  items[i].rtype.toString() ); //wo
     if(!items[i].field.isEmpty() )  row.setAttribute( "Field",  items[i].field );
...
void MainWindow::on_action_HTML_export_triggered()
{
  ...
  str += "<table border=\"0\" rules=\"none\" cellpadding=\"5\" cellspacing=\"0\" >\n";
  ...
  { str += "  <tr>\n";
    for(j=0;j<columns;j++)
    {
      ...
      int    alignment_role     = model->data( index, Qt::TextAlignmentRole ).toInt();
      ...
      {  disp_role = "<font color=\"" + foreground_role.toString() + "\">" + disp_role + "</font>";
      }

      str += "    <td";

      if( background_role.isValid() && background_role!=QColor(240,240,240) )
...
void MainWindow::on_action_about_triggered()
{
  QMessageBox::about( this, app_header,
     ...
     "Версия: " + ProgramInformation::version() + "<br><br>"
     "ЗАО \"Инкоммет\" <br><br>"
