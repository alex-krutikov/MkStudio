...
static int init()
{
    ...
    {
        cerr << "Configuration error:  Mode of operation (read or write) does "
                "not set.";
        print_help();
        return 1;
    }
    ...
    {
        cerr << "Configuration error: It is possible to write only a single "
                "bit.";
        print_help();
        return 1;
    }
...
static QString ptr_value(int offset, const char *ptr)
{
    ...
    case (Bytes):
        if (unsigned_mode)
        {
            return QString::number(
                (unsigned int)(*(unsigned char *)(ptr + offset)));
        } else
        {
    ...
    case (Words):
        ...
        {
            return QString::number(
                (unsigned int)(*(unsigned short *)(ptr + 2 * offset)));
        } else
        {
            return QString::number(
                (unsigned int)(*(short *)(ptr + 2 * offset)));
        }
        break;
    case (DWords):
        if (unsigned_mode)
        {
            return QString::number(
                (unsigned int)(*(unsigned int *)(ptr + 4 * offset)));
        } else
        {
